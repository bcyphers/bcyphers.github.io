...
Grid.prototype.movesAvailable = function () {
  return this.cellsAvailable() || this.pairsAvailable();
};
...
// Counts the number of potential pairs, or pairs of numbers which may be combined
// in one more turn. This is calculated by looking at all tracks on the board and 
// checking for a clear path in each between two like tiles. Tiles are weighted by 
// their value. 
Grid.prototype.pairsAvailable = function() {
  var pairs = 0;

  for (var x = 0; x < 4; x++) { // Iterate over every column
    for (var y = 0; y < 4; y++) {
      do {
        y++;
      } while (y < 4 && this.cellAvailable({x: x, y: y}))
      
      // If the next occupied tile is a match, skip to the next column.
      if (this.cellContent({x: x, y: y}) == content) {
        pairs += 1;
        break;
      }

      // Otherwise, we need to stay on the same tile. 
      y--;
    }
  }
  ...
  // Perform the same operation on the rows
  for (var y = 0; y < 4; y++) { 
    for (var x = 0; x < 4; x++) {
      content = this.cellContent({x: x, y: y}); // Check each tile
      if (content) {
        // If a tile is occupied, skip ahead to the next occupied tile and see if it matches
        do {
          x++;
        } while (x < 4 && this.cellAvailable({x: x, y: y}))
        
        // If the next occupied tile is a match, skip to the next row.
        if (this.cellContent({x: x, y: y}) == content) {
          pairs += 1;
          break;
        }
  ...
  return Math.max(pairs, colPairs);
}
...
Grid.prototype.monotonicity1 = function() {
  var self = this;
  var marked = [];
  var queued = [];
  var highestValue = 0;
